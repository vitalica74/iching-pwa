...
function hideRepeatedRationale(){
  ...
  const fullTransition=$('#transition-title')?.closest('details.knowledge-details');
  if(fullTransition){fullTransition.open=false;fullTransition.querySelectorAll('details[open]').forEach(item=>item.open=false);fullTransition.classList.add('structural-test-hidden-section')}
}
