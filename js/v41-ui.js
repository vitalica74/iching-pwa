...
function ensureStyles(){
  if(document.querySelector('#v41-reading-flow'))return;
  const style=document.createElement('style');style.id='v41-reading-flow';style.textContent=`
...
function formatMainCrossroads(){const action=$('#answer-action');if(!action||action.querySelector('.crossroads-main-path'))return;const text=action.textContent.trim();const marker='Це лише частина шляхів';const markerIndex=text.indexOf(marker);if(markerIndex<0)return;const pathsText=text.slice(0,markerIndex).trim();const openText=text.slice(markerIndex).trim();const paths=pathsText.split(/(?<=[.!?])\s+(?=[А-ЯІЇЄ])/u).filter(Boolean);if(!paths.length)return;action.replaceChildren();action.classList.add('crossroads-formatted');paths.forEach(path=>{const row=document.createElement('span');row.className='crossroads-main-path';appendCrossroadsPath(row,path);action.appendChild(row)});const open=document.createElement('span');open.className='crossroads-main-open';open.textContent=openText;action.appendChild(open)}
...
const libraryTarget=$('#library-detail');if(libraryTarget){let scheduled=false;const scheduleLibrary=()=>{if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;decorateLibraryLines()})};new MutationObserver(scheduleLibrary).observe(libraryTarget,{subtree:true,childList:true});scheduleLibrary()}
