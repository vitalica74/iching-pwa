...
function contextualize(text,context){if(context==='general')return ensurePeriod(text);const profile=CONTEXTS[context]??CONTEXTS.general;return `${profile.lead}: ${lowerFirst(ensurePeriod(text))}`;}
function summarizeLineFocus(lines){if(!lines.length)return 'Змінних ліній немає: ситуація радше просить правильно прожити наявний стан, ніж шукати різкий перелом.';const stages=joinNatural(lines.map(line=>line.stage));if(lines.length===1)return `Ключова зміна відбувається на рівні «${stages}».`;return `Зміни торкаються рівнів: ${stages}.`;}
function synthesizeAction(lines,primary,transition,context){if(!lines.length)return contextualize(primary.advice,context);if(lines.length===1)return contextualize(lines[0].advice,context);const first=lines[0]?.advice;const last=lines[lines.length-1]?.advice;const combined=first===last?first:`Почніть із нижчої активної точки: ${lowerFirst(first)} Далі врахуйте верхню межу змін: ${lowerFirst(last)}`;return contextualize(combined||transition.recommendation||primary.advice,context);}
function fallbackTransition(primary,secondary,hasChanges){if(!hasChanges)return {summary:`Стан «${primary.name}» залишається головним орієнтиром.`,change:'Окремого переходу до іншої гексаграми не показано.',recommendation:primary.advice,danger:primary.caution,future:'Подальший розвиток залежить від того, наскільки послідовно буде втілена порада основної гексаграми.',exact:false};return {summary:`«${primary.name}» переходить у «${secondary.name}».`,change:`Тема «${primary.name}» поступово відходить на другий план, а як наступний напрям проявляється «${secondary.name}».`,recommendation:primary.advice,danger:primary.caution,future:`Напрям розвитку: ${lowerFirst(secondary.desc)}`,exact:false};}
